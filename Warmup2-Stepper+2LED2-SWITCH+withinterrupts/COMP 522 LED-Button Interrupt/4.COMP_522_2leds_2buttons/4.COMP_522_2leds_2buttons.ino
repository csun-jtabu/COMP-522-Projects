/*
Jaztin Tabunda
Warmup Project 2 - 2 LED and 2 Buttons
COMP 522 - V. Bhatt
3-31-2023
*/
...
long unsigned int lastPress2; //this variable is going to keep track of the milliseconds since the last time the button2 was pressed
...
int pressCounter = 0; //press counter for both buttons
...
void loop()
{
  ...
  if( ((millis() - lastPress) > debounceTime && buttonFlag))
  {
    ...
    {
      pressCounter++;
      if(pressCounter == 1) //red on, green off
      {
      ...
      else if(pressCounter == 2) //green on, red off
      {
        toggleState =! toggleState; //toggleState controls the red led bulb from on to off and vice versa
      ...
      else if(pressCounter == 3) //both on
      {
        toggleState =! toggleState; //toggleState controls the red led bulb from on to off and vice versa
        digitalWrite(redLedPin, toggleState); 
      }
      ...
      {
        toggleState = 0; //turn off both leds
        toggleState2 = 0;
        digitalWrite(redLedPin, toggleState); 
        digitalWrite(greenLedPin, toggleState2);
        pressCounter = 0; //keeps track of button presses
      }
    }
    else if(digitalRead(buttonPin) == 1 && lastButtonState == 0)
    {
      lastButtonState = 1; //the current button2 state turns into the last button2 state the next time it's looped
    }
      buttonFlag = 0; //turns it to zero so it's not going to run the if statement again until button is pressed again.
  }
